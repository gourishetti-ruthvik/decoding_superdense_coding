...
def create_bloch_sphere_visualization(bit0, bit1):
    """Create cosmic 3D Bloch sphere visualization for quantum states"""
    import numpy as np
    
    # Define Bell states based on input bits
    states = {
        (0, 0): {"name": "|Φ+⟩", "color": "#00ff88", "theta": 0, "phi": 0},
        (0, 1): {"name": "|Ψ+⟩", "color": "#4fc3f7", "theta": np.pi/2, "phi": 0}, 
        (1, 0): {"name": "|Φ-⟩", "color": "#ff006e", "theta": 0, "phi": np.pi},
        (1, 1): {"name": "|Ψ-⟩", "color": "#8338ec", "theta": np.pi/2, "phi": np.pi}
    }
    
    current_state = states.get((bit0, bit1), states[(0, 0)])
    ...
    # Add cosmic Bloch sphere
    fig.add_trace(go.Surface(
        x=sphere_x, y=sphere_y, z=sphere_z,
        opacity=0.3,
        colorscale=[[0, 'rgba(79, 195, 247, 0.2)'], [1, 'rgba(106, 27, 154, 0.2)']],
        showscale=False,
        name="Bloch Sphere"
    ))
    
    # Add quantum state vector
    theta, phi = current_state["theta"], current_state["phi"]
    ...
    fig.add_trace(go.Scatter3d(
        ...
        mode='lines+markers',
        line=dict(color=current_state["color"], width=8),
        marker=dict(size=[8, 15], color=current_state["color"]),
    ...
    fig.add_trace(go.Scatter3d(
        x=[-axis_length, axis_length], y=[0, 0], z=[0, 0],
        mode='lines', line=dict(color='#ff006e', width=4),
    ...
    fig.add_trace(go.Scatter3d(
        x=[0, 0], y=[0, 0], z=[-axis_length, axis_length],
        mode='lines', line=dict(color='#4fc3f7', width=4),
    ...
    fig.update_layout(
        title=f"🌌 Quantum State: {current_state['name']} for [{bit0}{bit1}]",
        scene=dict(
